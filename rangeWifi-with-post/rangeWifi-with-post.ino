...
const char* version="rangeWifiHttp-with-post 2019-3-12";
...
//    Serial.println(message);
    server.send(200, "application/json", "{}");
    sendCommandWithData((char)F7CMD, message);
  } else {
    server.send(200, "application/json", "{}");
...
void sendCommandWithData(char command, String data) {
  SPI.transfer(command);
  debugMsgInt("sending command: ", command, true);
  SPI.transfer((char)data.length());
  debugMsgInt("sending length: ", data.length(), true);
  debugMsgStr("Sending message: ", data, true);
